...
class Slingshot(BaseStrategy):
    ...
    def next(self):
        ...
        is_strong = self.data1.close[0] > self.strong_sma[0]
        is_pullback = self.data1.close[0] < self.pullback_ema[0]

        # TODO only enter if not already in position
        if is_strong and is_pullback and slingshot:
            self.buy()
